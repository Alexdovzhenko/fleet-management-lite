...
const RANK_STYLES: Record<number, { bg: string; color: string }> = {
  ...
  2: { bg: "rgba(200,212,228,0.10)", color: "var(--lc-text-dim)" },
    ...
    >
      ...
      <div>
        {vehicles.map((vehicle, idx) => {
          const rank      = idx + 1
          const rankStyle = RANK_STYLES[rank] ?? { bg: "var(--lc-bg-glass)", color: "var(--lc-text-muted)" }
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1.5">
                  <div>
                    <span className="text-[13px] font-semibold" style={{ color: "var(--lc-text-primary)" }}>{vehicle.name}</span>
                    <span
                ...
                {/* Revenue bar */}
                <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "var(--lc-bg-glass)" }}>
                  <motion.div
                    className="h-full rounded-full"
                    style={{ width: `${barPct}%`, background: rank === 1 ? "#c9a87c" : rank === 2 ? "var(--lc-text-label)" : "#fb923c" }}
